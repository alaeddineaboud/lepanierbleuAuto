...
And ("l'utilisateur clique sur le bouton 'Ajouter au panier' pour le vendeur {string}", (vendeur) => {
  Cypress.on('uncaught:exception', (err, runnable) => {
    // returning false here prevents Cypress from failing the test
    return false
  })
...
And ("la pop-up se ferme automatiquement", () => {
  cy.get('.notifications-wrapper').contains('Vendu et Expédié Par').should('not.be.visible')
});

And ("l'utilisateur voit apparaître un bandeau de notifications {string}", (message) => {
